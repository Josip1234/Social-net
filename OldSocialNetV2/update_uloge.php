...
<body onMouseOver="prikazi_datum(),dohvati_kalendar()" , onLoad="slike()">
	...
	<div class="pravila">
		<section>
			...
			/*
while($ro=mysqli_fetch_assoc($t)){
	if(mysqli_num_rows($t)<2){
	      $t=$ro['uloga'];
		...
		die("Too many users select only one");
			...
			<form action="update_uloge.php" method="post">
				...
				<input type="text" name="uloga1" value="<?= $role; ?>"><br />
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				if ($t != $uloga1) {
					...
					if ($u) {
						...
						unset($_SESSION['selektiraj']);
						mysqli_close($dbc);
						header( "refresh:5;url=dodjelauloga.php" );
					} else {
