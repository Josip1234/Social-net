...
inner join databaseuser du on at.acTypeId=du.acTypeId where p.userId != :userId order by p.userId asc;";
  ...
  public static function getUserDataSearch(int $userId, int $limit, int $page, string $searchItems): array
  {
    $pattern = '%' . $searchItems . '%';
    $off = ($page - 1) * $limit;
    $db = Database::getInstance();
...
inner join databaseuser du on at.acTypeId=du.acTypeId having p.userId!=2 and userName like '%Korisnik%';
    
    
    */
    //not loggeddin user will not be printed (only admins can see this page so llogin admin cannot delete himself)
    $sql = "SELECT p.userId,concat(p.firstName,' ',p.lastName) as userName, p.email, p.dateOfBirth,pd.accountStatus, at.acTypeName, du.userName as databaseUser FROM profile p inner join profiledetails pd on pd.userId=p.userId inner join accounttype at on pd.acTypeId=at.acTypeId
inner join databaseuser du on at.acTypeId=du.acTypeId having p.userId != :userId and userName like :pattern order by p.userId asc LIMIT :lim OFFSET :off";
    $stmt = $db->prepare($sql);
    $stmt->execute([
      ':pattern' => $pattern,
      ':userId' => $userId,
  ...
  public static function selectCountRowsForUserDataSearch(int $userId, string $searchItems): int
  {
    $pattern = '%' . $searchItems . '%';
    $db = Database::getInstance();
    //not loggeddin user will not be printed (only admins can see this page so llogin admin cannot delete himself)
    $sql = "SELECT p.userId,count(p.userId) as userCount,concat(p.firstName,' ',p.lastName) as userName FROM profile p inner join profiledetails pd on pd.userId=p.userId inner join accounttype at on pd.acTypeId=at.acTypeId
inner join databaseuser du on at.acTypeId=du.acTypeId having p.userId != :userId and userName like :pattern";
    $stmt = $db->prepare($sql);
    $stmt->execute([
      ':pattern' => $pattern,
      ':userId' => $userId,
